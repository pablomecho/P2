...
int main(int argc, char *argv[]) {
  ...
  for (t = last_t = last_t1 = t1 = 0; ; t++) { /* For each frame ... */
    ...
    if (state != last_state && state != ST_UNDEF) {
      if (state==ST_VOICE) {
        if (t != last_t && (0.185 < (t * frame_duration-last_t * frame_duration))) { /*hacer que los fragmentos de silencio tengan una duracion minima*/
